...
if (isset($_POST['login'])) {
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        Your verification code is $code";
                    $sender = "From: techwebbeast99@gmail.com";
                    if (mail($email, $subject, $message, $sender)) {
                        $info = "It's look like you haven't verified your email. We've sent a verification code to your email - $email";
                        $_SESSION['info'] = $info;
                        $_SESSION['email'] = $email;
                        $_SESSION['password'] = $password;
                        header('location: user-otp.php');
...
if (isset($_POST['check-email'])) {
    ...
    if (mysqli_num_rows($run_sql) > 0) {
        $fetch1 = mysqli_fetch_assoc($run_sql);
        if($fetch1['status']=='verified'){
        $code = rand(999999, 111111);
        $insert_code = "UPDATE users SET code = $code WHERE email = '$email'";
        $run_query =  mysqli_query($db, $insert_code);
        if ($run_query) {
        ...
        echo "<script type = \"text/javascript\">
                    alert(\"This account is not yet verified. Please verify your account first.\");
	                window.location = (\"login-user.php\")
			  </script>";
    }
    } else {

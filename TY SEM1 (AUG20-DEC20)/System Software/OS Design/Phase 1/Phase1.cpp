...
//initiallise everything to 0 => everything will get cleared
void OS::init()
{
...
void OS::LOAD()
{
    ...
    int x = 0; //another variable pointing to memory
    do
    {
        ...
        if (buffer[0] == '$' && buffer[1] == 'A' && buffer[2] == 'M' && buffer[3] == 'J')   //control card
        {
            init(); //initialize
        }
        else if (buffer[0] == '$' && buffer[1] == 'D' && buffer[2] == 'T' && buffer[3] == 'A')  //program card
        {
            IC = 00;
            Execute();
        }
        ...
        else    //copy content from buffer to memory
        {
            int k = 0;
